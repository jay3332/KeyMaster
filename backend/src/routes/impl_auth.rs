...
impl FromRequest<Body> for Auth {
    ...
    async fn from_request(req: &mut RequestParts<Body>) -> Result<Self, Self::Rejection> {
        let authorization = req
            .headers()
            .ok_or_else(|| (500, Error {
                message: "Could not properly read headers.".to_string(),
            }))?
            .get(AUTHORIZATION)
            .ok_or_else(|| (400, Error {
                message: "'Authorization' header is required.".to_string(),
            }))?;

        let token = String::from_utf8(authorization.as_bytes().into()).map_err(|e| (400, Error {
            message: format!("'Authorization' header is not valid UTF-8: {}", e),
        }))?;

        let user_id = decode_token(token.clone()).ok_or_else(|| (500, Error {
